...
/*   Updated: 2024/03/22 15:04:09 by malanglo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_map_default_colors(t_map *map)
{
	...
	while (j_axis < map->total_height)
	{
		...
		while (i_axis < map->total_width)
		{
			value = percent(map->point_index[j_axis * map->total_width
					+ i_axis].z, z_min, z_max);
			map->point_index[j_axis * map->total_width
				+ i_axis].color = attribute_color_default(value);
			i_axis++;
		}
		j_axis++;
	}
